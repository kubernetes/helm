...
func newRollbackCmd(cfg *action.Configuration, out io.Writer) *cobra.Command {
	...
	f.BoolVar(&client.Force, "force", false, "force resources to be replaced by rendered templates on rollback. In combination with --recreate resources will be deleted before recreation.")
	f.BoolVar(&client.RecreateResources, "recreate", false, "patch resources, on error delete and recreate resources. In combination with --force always delete resources before recreation.")
