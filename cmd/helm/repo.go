...
func newRepoCmd(out io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "repo add|remove|list|index|update|validate [ARGS]",
		Short: "add, list, remove, update, validate, and index chart repositories",
		Long:  repoHelm,
		Args:  require.NoArgs,
	}
	...
	cmd.AddCommand(newRepoValidateCmd(out))
